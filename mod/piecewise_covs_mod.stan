...
model {
  // Prior on beta
  target += normal_lpdf(beta_ocs | beta_mean, beta_sd);
  target += normal_lpdf(beta_age | beta_mean, beta_sd);
  target += normal_lpdf(beta_sex | beta_mean, beta_sd);
  ...
  for (k in 1:K) {
    ...
    for (i in 1:N) {
      // Linear predictor
      real lp = beta_ocs * x_ocs[i] + beta_age * x_age[i] + beta_sex * x_sex[i];
